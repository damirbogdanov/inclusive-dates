...
describe('chrono parser', () => {
  ...
  describe('chronoParseDate', () => {
    ...
    it('supports strict mode behavior', async () => {
      let parsed = await chronoParseDate('today', { referenceDate, useStrict: true });
      expect(parsed).toEqual({ value: null, reason: 'invalid' });

      parsed = await chronoParseDate('2023-02-01', { referenceDate, useStrict: true });
      expect(isSameDay(parsed?.value, new Date('2023-02-01'))).toBeTruthy();

      parsed = await chronoParseDate('August 8 2004', { referenceDate, useStrict: true });
  ...
  describe('chronoParseRange', () => {
    it('parses natural-language ranges and single start dates', async () => {
      let parsed = await chronoParseRange('from january 25 to january 30', { referenceDate });
      expect(isSameDay(parsed?.value?.start, new Date('2023-01-25'))).toBeTruthy();
      expect(isSameDay(parsed?.value?.end, new Date('2023-01-30'))).toBeTruthy();

      parsed = await chronoParseRange('june 1984 - february 1990', { referenceDate });
      expect(isSameDay(parsed?.value?.start, new Date('1984-06-01'))).toBeTruthy();
      expect(isSameDay(parsed?.value?.end, new Date('1990-02-01'))).toBeTruthy();

      parsed = await chronoParseRange('june second 2008', { referenceDate });
      expect(isSameDay(parsed?.value?.start, new Date('2008-06-02'))).toBeTruthy();
    ...
    it('handles unsupported locales for ISO ranges', async () => {
      ...
      expect(isSameDay(parsed?.value?.start, new Date('2024-05-15'))).toBeTruthy();
      expect(isSameDay(parsed?.value?.end, new Date('2024-05-20'))).toBeTruthy();

      parsed = await chronoParseRange('fran 2024-07-01', {
        referenceDate,
        locale: 'sv',
      });
      expect(isSameDay(parsed?.value?.start, new Date('2024-07-01'))).toBeTruthy();
    ...
    it('supports custom expressions and defaults path', async () => {
      ...
      expect(isSameDay(parsed?.value?.start, new Date('2026-09-18'))).toBeTruthy();
    ...
    it('returns partially bounded ranges when at least one date is valid', async () => {
      ...
      expect(parsed?.value?.start).toBeNull();
      expect(isSameDay(parsed?.value?.end, new Date('2005-08-01'))).toBeTruthy();
    });
  });
});
