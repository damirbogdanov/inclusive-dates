...
describe('utils', () => {
  it('supports date arithmetic and navigation', () => {
    ...
    expect(getPreviousMonth(new Date('2022-01-20'))).toEqual(new Date('2021-12-20'));
  ...
  it('handles first/last day and ISO formatting', () => {
    expect(getISODateString(getFirstOfMonth(new Date('2022-01-20')))).toEqual('2022-01-01');
    expect(getISODateString(getLastOfMonth(new Date('2022-01-01')))).toEqual('2022-01-31');
  ...
  it('returns padded and unpadded month days', () => {
    ...
    const janStartIndex = paddedSunday.findIndex((d) => getMonth(d) === 1 && d.getDate() === 1);
  ...
  it('compares days and date ranges including guard clauses', () => {
    expect(isSameDay(new Date('2022-01-01'), new Date('2022-01-01'))).toBe(true);
    ...
    expect(isDateInRange(null as unknown as Date, { from: new Date(), to: new Date() })).toBe(false);
